...
class AttModel(CaptionModel):
    ...
    def _sample_beam(self, fc_feats, att_feats, imagelabels,att_masks=None, opt={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats,  imagelabels,att_masks)

        assert beam_size <= self.vocab_size + 1, 'lets assume this for now, otherwise this corner case causes a few headaches down the road. can be dealt with in future if needed'
        seq = fc_feats[0].new_full((batch_size * sample_n, self.max_seq_length), self.pad_idx, dtype=torch.long)
        seqLogprobs = fc_feats[0].new_zeros(batch_size * sample_n, self.max_seq_length, self.vocab_size + 1)
        ...
        # first step, feed bos
        it = fc_feats[0].new_full([batch_size], self.bos_idx, dtype=torch.long)
    ...
    def _sample(self, fc_feats, att_feats,  imagelabels ,att_masks=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, imagelabels,att_masks, opt)
        if group_size > 1:
